...
;
;	Last Change: TnT - Dec. 04, 1986 - 2.33 changes
;		     Tom - Jan. 19, 1987 - RSCMD() use CHKATR() & CHKOVR()
;		     Ted - Mar. 31 - Added YB and YO commands
;		     Tom - Apr. 01 - YSCMD:  _YS & p,qYS
;		     Ted - Apr. 03 - JOCMD fixed
;
;	A - APPEND
;
...
;
JOCMD:	CLR$	REGSTK		;;Handle BR8K() complication
	JMP	BR8K		;;Back to COMMAND: prompt
;
...
;
	IF	DEMO, [
	LXI	H,DEMMS2	;HL-> DEMO message
	JMP	MSGBRK

	] [

	LXI	D,AUXFCB
	CALL	CHKATR		;;BREAK out if r/o
	JRC	..1		;;Branch if no such file
	CALL	CHKOVR		;;Check if OK to overwrite
	RNZ			;;No, return
;
..1:	CALL	CREAUX		;BREAK if unable to create file
;
	IF	VPLUS, [
...
;
	...
	EPROC	RSCMD
;
; CHKOVR - Check with user whether OK to overwrite file.
;	   Return 'Z' if OK.
;
				;{RSCMD}
	BPROC	CHKOVR
CHKOVR:	CPIB$	RMINUS,'-'	;Skip query?
	RZ			;Yes, branch

	LXI	H,RSMSG		;HL-> message
	JMP	PREPLY		;Give prompt, get reply
	EPROC	CHKOVR
;
...
;
YICMD:	LXI	H,INSCHR	;Use insert character routine
	JMP	YIOCMD		;;Merge below to set/reset ROUTAD
;
...
..DWN:	DCR	A		;Decrement level count
	...
	EPROC	YMCMD
;
;	YO - Change ROUTAD to send text to OP sys; -YO restore.
;
YOCMD:	LXI	H,CONOUT	;Use console output to DOS
				;{YICMD}
YIOCMD:	CALL	CHKITR		;Is this 0Yx or -Yx?
	JZ	RSTOUT		;Yes, reset to console
	LDA	LSTFLG		;Use current LSTFLG
	JMP	SETOUT		;Set new ROUTAD to HL
;
;	YP - Set ROUTAD to print, -YP - change back to console.
;
...
;
YSCMD:	LXI	H,WSTRIP	;HL-> executive routine below
	JMP	GLOB0		;Process globally if required
;
	BPROC	WSTRIP
WSTRIP:	CALL	POSDIF		;Set HL > DE, BC = difference
...
;
	...
	INX$	H		;HL->.WWMODE
	MVI%ES	M,-1		;;Disable UPDVIS() in this window
...
;
	...
	CALL	OPNSRC		;Try to open the source file (BREAK if bad fn)
...
;
				;{EBCMD}
ERCMD1:
;;	TST$	INFLG		;Is an input file open?
;;	RZ			;No, return
