...
;
;	Last Change: Tom - Feb. 23, 1986 - 2.23a bug fixes
...
;		     Tom - Dec. 02 - KBUFFR()
;		     Ted - Mar. 31, 1987 - GETERN() saves ASCII value in REGASC
;
;***************************************************************************
;
; Memory layout:
...
;
;
; FREESP - Return BC = # free bytes, 'Z' set if none.
;
...
;
; PNTTRG - Point to actual text register/edit buffer (including length=0).
...
;6	MOVW	EVLGET+2,CMDGET+2
	CALL	EGETQR
	JMPR	GETCR9		;;Copy EVLGET to CMDGET
;
...
;
				;{MCMD,RCMDs,XCMD-,EECMD,YWCMD via GETCWN}
	BPROC	GETCRN
GETCRN:	MOVW	EVLGET,CMDGET
;6	MOVW	EVLGET+2,CMDGET+2
	CALL	GETERN
GETCR9:	MOVW	CMDGET,EVLGET
				;*** NOTE: Copy EVLCHR to CMDCHR?   *****
	RET
				;{GETCRN,RGET,UGET,WGET}
GETERN:	XRA	A		;Get zero
..1:	STA	REGAPP		;Set/reset appending flag
	CALL	ENXCUC		;Get next character; AL = UC
	CPI	'+'		;Appending?
...
; SETRN - Set REGNUM.
	...
	BPROC	SETRN
SETRN:	STA	REGASC		;;Save ASCII value for YWCMD, WGET
	CALL	CONVRG		;Convert register # to binary
...
;
	...
	JZ	CANCL1		;Yes, cancel and close status line
;
