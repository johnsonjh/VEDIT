...
;
;	Last Change: Ted - Nov. 03 - Fix VDELET bug
...
;		     Ted - Apr. 06, 1987 - LINETG fix for empty line
;
...
VGODEC:	DB	'B'		;Begin of File
	...
	DSEG	$
VGOPMT:	DC	'[B]egin  [H]ome  [S]et  [J]ump  [L]ine  [Z]end  [E]nd ' [00]
...
JVSMAC:	JMP	VISMAC		;
;
; GOSET - Save current line position
;
GOSET:	CALL	GETMNM		;Get marker number
...
..1:	JMPR	FNDEXE		;Search for the string
	EPROC	VFIND
...
..ERR:	LXI	H,CNFMSG	;HL-> "CAN NOT FIND"
	...
	CALL	MSGHND		;Display message on status line
	JMP	MACRST		;;Throw away any keystroke macro
;
..OK:	LDA	FNDFLG		;Get find mode
	CPI	3		;Replace "Rest"?
	JRZ	VRPEX4		;Yes, branch
	CPI	2		;Replace with prompt?
	JRZ	VRPEXE		;Yes, branch
	RET			;No, all is done
	EPROC	VFNEXE
	.PAGE
;
...
LINETG:	CALL	ZIPCHK		;At end of line or end of window?
	...
	CPIW	ACTPNT,ACTBUF	;At begin of active line?
	RZ			;;Yes, do nothing on an empty line
				;Else perform BACKTB
;
