...
;			   Jan. 29, 1986 - PRTDEC used ITOA
;			   Feb. 02, 1986 - ++ direction for command buffer
;
;		     Tom - Mar. 09, 1986 - VEDIT/VPLUS combined
;
;		     Ted - Mar. 23, 1986 - Enable OVERWT
;			   Mar. 24, 1986 - Redo INSLEN,INSCHR,INSBLK
;					   No longer set MINUS if ITRCNT == 0
;
;		     Tom - Apr. 08 - PSHCMD save BDH for MCMD ...
;			 - Apr. 21 - PSHCHK at end of PSHCMD from C1
;			 - May. 06 - BAKLIN set DE=TXTFLR, necessitated by
;				     Ted's "optimization" of FNDLIN
;			 - May  08 - GLBCMD removed (see C2 for new PRCGLB)
;			 - May  09 - PAKTX0, PKTX00 entry points added to PACKTX
;				     SCCMPS() now JMPR PAKTX0 (global K cmd)
;			 - May  13 - ERRHND(2) for YM error (matching group delimiter not found)
;		     Ted - Oct. 06 - ERRHND checks FINDPT and REPLFL
;		     Tom - Oct  07 - CHKITR() bug
;		     Ted - Oct. 24 - Inserted HELPEX() from C2
;			 - Dec. 05 - HELPEX() uses STRBUF[]
;
...
ERRHND:	TST	REPLFL		;Is this a [FIND] or [REPLACE]?
	...
	RET			;Thats all for [FIND] or [EPLACE]
;
..1:
	IF	VPLUS, [
...
;
	...
	JRM	ERRBR8		;Yes, error, BR8K
;
	PUSHF			;Save result
	LXI	H,CNFMSG	;HL-> 'CANNOT FIND...'
	LXI	D,TARGST	;HL-> search string
	POPF			;NOTSUC?
	JRZ	..2		;Yes, branch
	LXI	D,GRPPAT+1	;HL-> corresponding group delimiter
..2:

	IFNOT	VPLUS, [
	LDA	TERMCH		;Get terminator
	MOV	C,A		;End message on terminator
	]

	IF	VPLUS, [
	MVI	C,EOF		;End message on terminator
	]
	MVI	B,0
...
ENDITR:	CALL	CHKLVL		;Pop clauses from this command level.  Any?
	RC			;No, return
	MOV	A,D		;Yes, but what kind of clause is it?
	ORA	E		;THEN clause?
	BEQ	ENDITR		;Yes, keep popping
	DCX$	D		;GET = 1 => ELSE clause
	MOV	A,D
	ORA	E		;ELSE clause?
	BEQ	ENDITR		;Yes, keep popping
	INX$	D		;DE-> past current loop begin
	SDED$	CMDGET		;So we can properly scan past this loop
	lda	regexn
	call	pshitr
	call	scngit
	jmp	popitr
...
;
..1:	LXI	D,STRBUF	;Use GP buffer for user response
