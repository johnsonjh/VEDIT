...
;
	IF	DEMO, [
DEMMSG:	DC	'DEMO VERSION DOES NOT SAVE FILES ON DISK' [00]
	]
;
;
	IF	VPLUS, [
...
RENUSR:	DS	1		;User # for REN FCB
	...
	IF	MSDOS, [
	DB	0FFH		;Extended FCB flag
	DB	0,0,0,0,0	;MSDOS reserved bytes
	DB	3H		;Hidden & R/O attributes
	]
...
;
	IF	MEMVRS, [
BANKFL:	DB	00		;Screen bank selected. (0=No)
	]
...
;6MCB	DW	0		;Memory Control Block
...
PHYHOR:	DS	1		;CRT cursor horizontal pos (CRTVER above)
;
;
; WSTRUC - Window structure table
;
;	   .WWNAME -  Name
;	   .WWMODE -  Mode, 0=command, 1=visual
...
WWMODE:	DB	0		;0 = command, Edit-buf # = visual
...
ACTBUF:
;
INIMS1:	DC	'Copyright (C) 1986 CompuView Products, Inc.' [CR] [LF] [CR] [LF]
;
	IF	VPLUS, [
...
USRMSG:	DC	'Welcome to VEDIT '
	...
	ORG	USRMSG + 65
;
	DC	'Copyright (C) 1979,1986 by Theodore Green.'
	DC	'12/21/86' [ESC]
;
...
AUXUSR:	DS	1		;Auxiliary-file user #
	...
	IF	MSDOS, [
	DB	0FFH		;Extended FCB for showing hidden files (FCB-7)
	DB	0,0,0,0,0	;MSDOS reserved bytes
	DB	12H		;Attributes hidden and directory
	]
