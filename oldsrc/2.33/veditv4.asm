...
PRTCHA:	MOV	C,A		;Save char in C
	...
	LDA	BIT7AL		;Get EP parameter value
...
;
	...
	ORA	A		;Was this window last used for command mode?
	JRZ	..1		;Yes, branch
;
	...
	IFNOT	WINDOO, [
	CLR$	WWMODE		;Window now used for command mode
	]
;
...
;
..3:	TST$	VISFLG		;Is this visual mode?
...
;
	CPIB$	WWNAME,01	;Is this the status line?
...
..1:	DCR	L		;Backup to previous line
	...
	.PAGE
;
; ATTOFF - Turn screen attributes off immediately.
;
				;{WINEOS,WINEOL,WININI,WIBTOP}
ATTOFF:
	IF	CRTVRS, [
	LXI	H,OUTATT	;HL-> routine to change CRT attribute
	PUSH	H		;OUTATT() will be executed after ATTRST
	]
;
;	ATTxxx - Change attribute for next write.
;
...
WININI:	LD16	H,SFORAT	;Load INSTALL attributes
	...
	CALL	ATTOFF		;Restore to background attribute
;
...
;
	...
	ST16	STATWW+10,H	;Store
;
	CALL	WININ0		;;Init rest of window variables
	JMP	STACLR		;Clear the status line
;
				;{YWDELE}
WININ0:	CALL	WIZOOM		;Set current window to full screen
;
	IF	WINDOO, [
...
;
	...
	ORA	A		;Does window contain visual mode?
	JRZ	..3		;No, just write border for window
;
...
;
; SWSTAT - Switch to status-line (fake) window; WWNAME = 01H.
...
..2:	CALL	HZENST		;Set HZSCEN from HZSCBG and WWLLEN
	CPIB$	WWNAME,1	;Is this the status line?
...
;
	BPROC	YECMD
YECMD:	CALL	NXCMUC		;A = next command char; convert to UC
...
;
	BPROC	YWCMD
YWCMD:	CALL	NXCMUC		;A = next command char; convert to UC
	CPI	'Z'		;Zoom?
	JZ	YWZOOM
	CPI	'I'		;Init?
	JZ	WININI
...
..1:	STA	WINTYP		;Save T/B/L/R as flag
	...
	CALL	NXCMUC		;AL = next command char; convert to UC
...
;
..2:	CPIB$	WINTYP,'D'	;Is this window delete?
...
..ERR:	CALL	WINCHK		;May need to reset "full" window
	...
	EPROC	YWCREA
;
; YWDELE - Delete the current window, recreate all existing windows.
;
	BPROC	YWDELE
