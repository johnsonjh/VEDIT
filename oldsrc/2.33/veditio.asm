...
;
;	Last Change: Ted - Sep. 24, 1985 - Changes for 2.32
;			   Nov. 13, 1985 - Changes for new keyboard decode
;		     Tom - Jan. 31 - KBINSW (ES # 12) determine input routines
;		     Ted - Feb. 06 - Change KEYTBL to use memory manag.
;			   Feb. 28 - CONOUT and LSTOUT save BC, DE and HL
;			   Mar. 24 - Use INPFLG in GETCHR
;			   May  10 - LSTOUT performs formatted output
;			   May  10 - GETKEY() return char without decoding
;			   June 03 - GETCHR() blinks MM cursor
;			   July 29 - Repeat functions
;			   Sep. 18 - GETCHR to check cursor position
;			   Oct. 12 - GETCHR calls CHKBRD and UPDVIS
;			   Oct. 13 - Fix DEFINE - needed CALL STADON
;			   Oct. 28 - 8 bit/FUNC/CTRL-key handling
...
;			   Dec. 09 - DECKEY and GETCHR return "VN" if undefined
;
...
;6	MOV	AL,BPT ES:LOW418	;;Get keyboard status
...
;
..1:	CALL	CURSON		;Turn the cursor on
...
..3:	MOV	A,M
	...
	JMPR	..3
;
;	Check if control-sequence valid.
;
...
;
	BPROC	LRNDEL
LRNDEL:	LXI	H,LECONF	;HL-> confirmation prompt
	CALL	STAKEY		;Give prompt, get reply, AL = UC
	CPI	'Y'		;
	STC
...
;
	LES	H,SEQADD	;ES:HL-> begin of control-sequence
