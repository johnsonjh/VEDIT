...
;
	...
	CPI	'Z'+1		;Command?
	BGE	EXPRR		;No, continue with evaluation
	RET			;Yes return
;
EXPRR:				;{EXPR4}
...
XPROR:	CALL	EGTSCN		;A = terminator of 1st expression
	...
	JMP	XPROR		;Look for more OR terms
;
...
XP25:	CALL	NXTBDE		;BDE = e2, CHL = e1
	...
	JMP	XP23		;Keep checking for more terms
;
...
XP33:	DAD	D		;Multiply by adding one term
	...
	JNZ	XP33
	JMP	XP35		;Adding done, now go handle signs
;
;	Division
;
XP34:	CPI	'/'		;Division?
...
XP35:	MVI	C,0		;Positive sign for absolute arithmetic
	...
	JMP	XP31		;Go look for more terms
;
;	Remainder
;
XP36:	CPI	'%'		;Remainder?
...
;***************************************************************************
;
; TSTV - Check for/return variable value in CHL.  'NC' for success, else 'C'.
;	 Returns EVLGET -> next scan char.
;
...
;	     Q0-Q25 are user-definable variables.
;	.v = value returned by routine "v" listed in table VALTBL.
;
				;{EXPR4}
TSTV:	CALL	EGTSCN		;A = current character
	CPI	'#'		;Check for #
	BNE	TSTV2
...
TSTV1:	MVI	C,0
	JMPR	TSTYES
;
TSTV2:	CALL	TSTCTL		;Control code ^a ?
	RZ			;
	CALL	TSTASC		;ASCII character "a ?
	RZ			;
	CALL	TSTVAL		;Valuator routine .v ?
	RZ
				;No, check for Q-register
...
TSTCT1:	MOV	L,A		;L = ASCII code
	...
	JMPR	TSTV1
;
; TSTASC - Check command buffer for "a, return L = ASCII value(a).
;	    'Z' if successful, else 'NZ'.
;
;	    INPUT: A = current scan character.
;
				;{TSTV}
TSTASC:	CPI	'"'		;Double quote mark?
	RNZ			;No, then return 'NZ'
	CALL	ENXCHR		;Yes, A = next command character
	JMPR	TSTCT1		;Proceed as with CHKCTL
;
; TSTVAL - Check command buffer for .v, return CHL = value.
;	    'Z' if .v format, else 'NZ'.
;
				;{TSTV}
TSTVAL:	CPI	'.'		;Period?
	RNZ			;No, then return 'NZ'
...
EXEC1:	PUSH	D		;Save start of string in case of no match
EX1:	LDX%ES	D		;A = next character in string
	INX$	D		;DE -> past current command line char
	INX$	H		;HL -> char in table being searched against
	CMP	M		;Match?
	JZ	EX1		;Yes, so far so good
	...
	BEQ	EX5		;Yes, pick up executive routine's address
EX2:	INX$	H		;No, scan past rest of string in table
	CMP	M		;Reached the end of table's current string?
	BNE	EX2		;Not yet, keep looking
	INX$	H		;Advance HL past executive routine address
	INX$	H		;
	POP	D		;DE -> start of command-line string
	JMPR	EXEC1		;Try table's next string
EX5:	INX$	H		;Bump pointer past delimiter
...
;
EGET:	CALL	ENXCUC		;A = next scan char; convert to UC
	CPI	'F'		;Checking for EOF?
	BNE	ERGET		;No, check for error flag
;
;	Check for EOF.
;
	CALL	INFLCH		;EOF?
...
;	Get error flag.
;
ERGET:	CPI	'R'		;Error flag?
	BNE	ESGET		;No, check for Search Error flag
	LHLD	ERRFLG		;HL <= error flag
	JMPR	RETHL
;
;	Get Search Error flag.
;
ESGET:	CPI	'S'		;Search Error flag?
	BNE	EWGET		;No, check for Write Error
	LDA	SRCERR
JRETAF:	JMPR	RETPSW
;
;	Get Write Error flag.
;
EWGET:	CPI	'W'
	LDA	WTERFL
				;{EWGET^,OGET}
...
FGET:	CALL	FREESP		;BC = # free bytes
	JMPR	RETBC
;;
;;	LM - Get current left margin.
;;
LGET:	CALL	ENXCUC		;;A = next scan char; convert to UC
	CPI	'M'		;;Left margin?
	JNZ	VALBRK		;;No, error
	LDA	INDPOS
	JMPR	RETPSW
;
;	M - Get current command execution buffer number
;
MGET:	LDA	REGEXN
	JMPR	RETASC
;
...
NGET:	LHLD	NFOUND
	JMP	TSTV1
;
;	OF - Output File Open Flag
;
OGET:	CALL	ENXCUC		;;A = next scan char; convert to UC
	CPI	'F'
	LDA	OUTFLG
	JMPR	IFRETF
;
...
;
	BPROC	RGET
RGET:	CALL	ENXCUC		;;A = next scan char; convert to UC
	CPI	'M'		;Remainder?
	BNE	..RTGT		;;No, branch
...
;	Return HL = [BLMVEN].
;
VMGET:	CALL	ENXCHR		;Advance GET pointer
	LHLD	BLMVEN
	JMPR	PGET0
;
;	W - Get Window Coordinates.
;
	BPROC	WGET
WGET:	CALL	ENXCUC		;;A = next scan char; convert to UC
