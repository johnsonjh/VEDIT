...
;
	...
	JRNZ	OUTAT1		;Yes, turn it off
	LXI	H,ESCVDR	;No, HL-> escape char. table
OUTAT1:	CALL	CRTCRL		;Turn reverse video on/off
;
OUTAT2:	MOVB	ATTRFL,ATTRIB	;Save new screen attribute
	POP	B
	RET
;
...
;
	...
	POPA			;Restore count
;
WRTAD1:	MVI	C,'0'		;Init tens counter to '0'
WRTAD2:	INR	C		;Tens = tens + 1
	SUI	10		;Subtract ten
	JRNC	WRTAD2		;Branch until underflow
;
	...
	JRZ	WRTAD4		;Yes, don't send zero to CRT
	CALL	OUTSAV		;Send the tens count to CRT
;
WRTAD4:	POPA			;Restore Ones digit
...
;
;	Could check here if window is entire screen - send LF and
;	set to rewrite status line.
;
				;{WTSCRL,WINLF}
	BPROC	WISFWD
WISFWD:	CALL	ATTOFF		;Make sure erase attribute is set
	TST$	ESCINS		;;Does CRT have insert line?
	JRNZ	..1		;;Yes, branch
	LD16	H,PYLINE	;;L = bottom line #
	CALL	PHYST0		;;Address bottom screen line
	CALL	CRTFSL		;;No, scroll entire screen forward
	CALL	STBRFL		;;Flag to rewrite status line
	CALL	STNSNL		;;Make sure VS window rewritten
				;;Needed when enter VS window with V command
	JMPR	..2		;;Merge below
;
...
;		  WWUSLN updated.
;
WISLBK:	CALL	ATTOFF		;Make sure erase attribute is set
...
;
	...
	CALL	ATTFOR		;;Back to foreground vidoe
...
;
	CALL	ATTOFF		;Yes, force window erase attribute
...
;
WINEOS:	CALL	ATTOFF		;Force window erase attribute
...
;
WINEOL:	CALL	WIEOLC		;Can physical EOL be performed?
	JRC	WIEOL2		;;No, branch
	LXI	H,ATTFOR	;;&Routine to restore text attribute
	PUSH	H		;;Make sure text attribute restored
	CALL	ATTOFF		;;Force window erase attribute
...
WIEOL3:	JZ	PHYRST		;When done, reset cursor pos
	CALL	OUTSAV		;Send a space to the CRT
	DCR	B		;Decrement count
	JMPR	WIEOL3		;Continue
;
PHYRST:	LD16	D,PHYVER	;Get desired cursor pos
	MVIW$	PHYVER,-1	;Make sure cursor gets positioned
	XCHG			;HL = desired pos
...
CRTCR1:	INX$	H		;HL-> escape char
	MOV	C,M		;Get the next char
	CALL	OUTSAV		;Send to CRT, HL is saved
	DJNZ	CRTCR1		;Continue sending bytes
;
	POP	B		;Restore BC
	INX$	H		;HL-> delay byte
...
VEXIT:	CALL	CRTEXT		;Reset CRT with escape sequences
	...
	CALL	ATTOFF		;Make sure exit in normal video
;
